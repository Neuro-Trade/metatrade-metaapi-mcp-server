...
function createMCPServer(sessionId) {
    ...
    // List prompts
    newServer.setRequestHandler(ListPromptsRequestSchema, async () => {
        return {
            prompts: [
                {
    ...
    // Get prompt
    newServer.setRequestHandler(GetPromptRequestSchema, async (request) => {
        const { name, arguments: args } = request.params;
...
async function main() {
    ...
    app.get('/health', (req, res) => {
        res.json({ status: 'ok', server: 'MetaAPI MCP Server' });
    });
    ...
    app.get('/sse', async (req, res) => {
        ...
        try {
            ...
            logger.info(`SSE connection established with session: ${sessionId}`);
    ...
    // Message endpoint - receives POST messages from client (with session ID in path)
    app.post('/message/:sessionId', async (req, res) => {
        const sessionId = req.params.sessionId;
        logger.info(`POST /message/${sessionId}`);
    ...
    app.listen(PORT, () => {
        logger.info(`MetaAPI MCP Server running on http://localhost:${PORT}`);
